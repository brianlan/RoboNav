...
def _checkpoint_non_reentrant_default(func, *args, **kwargs):
    kwargs.setdefault("use_reentrant", False)
    return _orig_checkpoint(func, *args, **kwargs)


cp.checkpoint = _checkpoint_non_reentrant_default
...
from prefusion.utils.config_confirmation import (
    collect_confirmation_snapshot,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="skip the interactive confirmation prompt before training",
    )
...
def main():
    ...
    skip_confirmation = args.skip_train_confirmation or os.environ.get("SKIP_TRAIN_CONFIRMATION") == "1"
    _maybe_confirm_training(cfg, skip_confirmation)
...
def _maybe_confirm_training(cfg: Config, skip_confirmation: bool) -> None:
    if skip_confirmation:
        return

    snapshot = collect_confirmation_snapshot(cfg)
    table = render_confirmation_table(snapshot)
    print(table)
    ...
    if not sys.stdin.isatty():
        raise SystemExit(
            "[confirmation] Interactive confirmation required. Pass --skip-train-confirmation or set SKIP_TRAIN_CONFIRMATION=1 to bypass."
        )

    while True:
        try:
            reply = input("Proceed with training? [Y/n]: ")
        except EOFError:
            raise SystemExit("[confirmation] Confirmation aborted (no input).") from None
