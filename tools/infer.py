...
def parse_args():
    ...
    parser.add_argument(
        "--auto-convert-to-weights-only",
        default=False,
        action="store_true",
        help="Automatically convert the checkpoint to weights-only format if it is not already in that format",
    )
...
def main():
    ...
    cfg.merge_from_dict({"visualizer": None})  # disable visualizer during inference.
    ...
    if args.auto_convert_to_weights_only:
        ...
        try:
            torch.load(cfg.load_from, weights_only=True)
        except pickle.UnpicklingError as e:
            if "weights_only" in str(e):
                tmpdir = mktmpdir()
                a = torch.load(cfg.load_from, weights_only=False)
                b = {"state_dict": a["state_dict"]}
                tmp_cp_save_path = tmpdir / Path(cfg.load_from).name
                torch.save(b, tmp_cp_save_path)
                cfg.load_from = str(tmp_cp_save_path)
                print_log(
                    f"Input checkpoint file is not weights-only. We're converting it to weights-only and saved at {tmp_cp_save_path}",
                    logger="current",
                    level=logging.WARNING,
                )
