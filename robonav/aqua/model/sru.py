...
Minimal port of ``rsl_rl.networks.sru_memory.lstm_sru`` (ETH Zurich, BSD-3-Clause).
Same gate equations, parameter layout, and state_dict keys as upstream.
"""
...
__all__ = ["LSTMSRUCell", "LSTM_SRU", "SRU"]
...
class LSTMSRUCell(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, bias: bool = True) -> None:
        ...
        if bias:
            with torch.no_grad():
                self.linear_all.bias[hidden_size : 2 * hidden_size] = 1.0 + torch.randn(hidden_size)
...
class LSTM_SRU(nn.Module):
    ...
    def forward(
        ...
        if state is None:
            state = self.init_state(batch_size, x.device, x.dtype)
...
# Local generic name for the same module.
SRU = LSTM_SRU
